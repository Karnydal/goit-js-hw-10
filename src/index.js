...
const containerList = document.querySelector('.country-list');
...
function createText(data) {
  ...
  if (data.length >= 2 && data.length <= 10) {
    ...
    containerList.insertAdjacentHTML('beforeend', markupList);
  }
  if (data.length === 1) {
    const markupInfo = textInfo(data);
    containerInfo.insertAdjacentHTML('beforeend', markupInfo);
  }
}
